...
However, some provide functionality which can outweighs these downsides in certain situations, particularly when it comes to [content blocking](basics/common-threats.md#mass-surveillance-programs).

Be particular about the extensions on this page you choose to install, and don't install anything which duplicates the functionality of another tool/extension you're already using, or don't immediately have a need for. For example, [Brave](desktop-browsers.md#brave) users don't need to install uBlock Origin, because Brave Shields already provides the same functionality.
